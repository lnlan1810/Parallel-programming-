...
void ProcessInitialization(double*& pMatrix, double*& pVector, double*& pResult,
    double*& pProcRows, double*& pProcVector,
    ...
    if (ProcRank == 0) {
        do {
            ...
            if (scanf_s("%d", &Size) != 1) {
                ...
                printf("Size must be greater than or equal to the number of processes! \n");
            }
        } while (Size < ProcNum);
    }
...
void DataDistribution(double* pMatrix, double* pProcRows, double* pVector,
    ...
    // Define the disposition of the vector elements for current process
    RestRows = Size;
    pProcInd[0] = 0;
...
void ParallelEliminateColumns(double* pProcRows, double* pProcVector,
    double* pPivotRow, int Size, int RowNum,
    int Iter) {
    double multiplier;
    for (int i = 0; i < RowNum; i++) {
